...
impl Solution for Day10 {
    fn part_1(&self, input_file: &str) -> String {
        ...
        contents.lines().map(|l| if let CheckResult::IllegalChar(c) = check_line(l) {
            match c {
                ')' => 3,
                ']' => 57,
                '}' => 1197,
    ...
    fn part_2(&self, input_file: &str) -> String {
        let contents = fs::read_to_string(input_file).unwrap();
        let mut costs = contents.lines().map(|l| if let CheckResult::Completion(s) = check_line(l) {
          get_completion_cost(&s)
        } else {
            0
        }).filter(|cost| *cost != 0).collect_vec();
        costs.sort();
        let mid = costs.len() / 2;
        costs[mid].to_string()
    }
